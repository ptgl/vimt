...
namespace Labixa.Models
{
    public static class LoginClass
    {
            ...
            try
            {
                ...
                ))
                {
                    ...
                    while (dr.Read())
                    {
                        ...
                        member.shortId = name;
                        member.ManagerWWID = dr["MgrWWID"].ToString();
                    }
                    sqlconnection.Close();
                }
                return member;
        ...
        public static Member GetName(int wwid)
        {
            try
            {
                ...
                {
                    string query = "SELECT ccMailName " +
                        "FROM WorkerPublicExtended (nolock) " +
                        "WHERE (LocCountryName = 'VIET NAM' AND BadgeType = 'BB' AND WWID = '" + wwid.ToString() + "') ";
                    SqlCommand sqlCommand = new SqlCommand(query, sqlconnection);
                    //sqlCommand.Parameters.AddWithValue("@A", "Value");
                    sqlconnection.Open();
                    SqlDataReader dr = sqlCommand.ExecuteReader();
